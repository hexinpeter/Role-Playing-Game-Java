...
public abstract class Uncontrollable extends Unit
{
  ...
  public void move(int xMovement, int yMovement, int delta, float speed, World world) {
    ...
    if (!world.hasBlocking(xPosTest, (int)yPos) && yMovement == 0)
      xPos = xPosTest;
   	else if (!world.hasBlocking((int)xPos, yPosTest) && xMovement == 0)
      yPos = yPosTest;
    else if (xMovement != 0 && yMovement != 0 && !world.hasBlocking(xDiagonal, (int)yPos) && !world.hasBlocking((int)xPos, yDiagonal)){
    	xPos = xDiagonal;
    	yPos = yDiagonal;
    }
